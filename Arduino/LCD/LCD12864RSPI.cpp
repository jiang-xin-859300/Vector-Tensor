...
void LCD12864RSPI::WriteByte(int dat) //写入字节
{
    digitalWrite(latchPin, HIGH);   //latchPin 高时，选择数字寄存器，latchPin低时，选择指令寄存器
    delayns();
...
void LCD12864RSPI::WriteCommand(int CMD)  //写入指令
{
   int H_data,L_data;
   H_data = CMD;
   H_data &= 0xf0;           //屏蔽低4位的数据
   L_data = CMD;             //xxxx0000格式
   L_data &= 0x0f;           //屏蔽高四位的数据
   L_data <<= 4;             //xxxx0000格式
...
void LCD12864RSPI::WriteData(int CMD)   //写入数据
{
   int H_data,L_data;
   H_data = CMD;
   H_data &= 0xf0;           //屏蔽低4位的数据
...
void LCD12864RSPI::Initialise()   //初始化
{
    pinMode(latchPin, OUTPUT);     
    pinMode(clockPin, OUTPUT);    
    pinMode(dataPin, OUTPUT);
    digitalWrite(latchPin, LOW);  //latchPin,拉低，选择指令模式
    delayns();
...
void LCD12864RSPI::CLEAR(void)  //清屏
{  
    WriteCommand(0x30);//
...
void LCD12864RSPI::DisplayString(int X,int Y,uchar *ptr,int dat)  //显示字符
{
  int i;
...
void LCD12864RSPI::DrawFullScreen(uchar *p) //绘制整个屏幕
{
      int ygroup,x,y,i;
      int temp;
      int tmp;
             
      for(ygroup=0;ygroup<64;ygroup++)  //写入液晶上半图像部分
        {                           //写入坐标
           if(ygroup<32)
            {
             x=0x80;
